...
@[simp] theorem Option.some_bind {α β : Type} (a : α) (g : α → Option β) :
    (Option.some a >>= g) = g a :=
  by
    simp [Bind.bind, Option.bind]  -- [Question] Why do we need `Bind.bind`?
  ...
  { FAction.Bind, FAction.Pure with
    pure_bind :=
      by
        intro α β ma f
        apply funext
        simp [Bind.bind, Pure.pure, FAction.bind, FAction.pure, Option.bind]
    bind_pure :=
      by
        intro α ma
        apply funext
    ...
    bind_assoc :=
      by
        ...
        apply funext
        intro x
        simp [Bind.bind, FAction.bind, Option.bind]  -- [Question] Why can `Option.bind` be omitted but not `FAction.bind`?
        cases ma x <;> rfl
  }
...
theorem pure_kleisli {m : Type → Type} [LawfulMonad m] {α β : Type}
  ...
  by
    apply funext
    simp [kleisli, LawfulMonad.pure_bind]
...
theorem kleisli_pure {m : Type → Type} [LawfulMonad m] {α β : Type}
  ...
  by
    apply funext
    simp [kleisli, LawfulMonad.bind_pure]
...
theorem kleisli_assoc {m : Type → Type} [LawfulMonad m] {α β γ δ : Type}
  ...
  by
    apply funext
    simp [kleisli, LawfulMonad.bind_assoc]  -- [Question] What does `simp [kleisli]` do?
    simp [<-kleisli.eq_1]
